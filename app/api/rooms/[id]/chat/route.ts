...
import { sendChat, sideForToken, viewFor } from '@/lib/game/match';
...
export async function POST(request: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  ...
  const outcome = await mutateRoom(roomId, (state) => {
    ...
    const result = sendChat(state, side, body.text, Date.now());
    return result.ok ? { ok: true as const, value: side } : result;
  });
